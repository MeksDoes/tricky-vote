...
describe('PollController', () => {
  ...
  const mockPollService = {
    ...
    update: jest.fn().mockImplementation((pollId, dto) => {
      return {
        pollId,
        ...dto,
      }
    }),
    remove: jest.fn().mockImplementation((pollId) => {
      return {
        pollId
      }
    }),
    findOne: jest.fn().mockImplementation((pollId) => {
      return pollId
    }),
  ...
  const stubPollDto = new CreatePollDto(
    'any title',
    'any question',
    [new OptionDto('Option 1'), new OptionDto('Option 2')],
  );
  ...
  describe('polls', () => {
    ...
    it('should update a poll', () => {
      ...
      expect(mockPollService.update).toHaveBeenCalledWith(stubPollId, { question: questionStub });
    });
